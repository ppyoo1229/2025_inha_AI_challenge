...
class Config:
    def __init__(self):
        ...
        self.MAX_TRAIN_STEPS = 300 # 총 학습 스텝 수 (None이면 EPOCHS로 계산)
        ...
        self.VAL_INTERVAL = 1 # 1 에폭마다 검증 (현재는 SAVE_AND_VAL_INTERVAL로 대체됨)
        self.SAVE_AND_VAL_INTERVAL = 10 # 스텝 기반 검증 및 저장 주기
...
class PromptEnhancer:
    def __init__(self):
        self.fixed_tail = "high detail, photorealistic, real people, sharp, natural, original color, neutral, balanced tone, "\
            "actual person, preserve structure, maintain faces, accurate color, not oversaturated"
        self.base_negative_prompts = "bad quality, vivid, cartoon, sketch, monochrome, grayscale, low detail, deformed, distorted, missing face, blurry, "\
            "overexposed, oversaturated, too bright, low contrast, artificial, neon, unrealistic, burnt, posterization, color artifact"
...
def train_loop(
    ...
    for epoch in range(first_epoch, cfg.EPOCHS):
        ...
        for step_in_epoch, batch in enumerate(train_dataloader):
            ...
            if (global_step % cfg.SAVE_AND_VAL_INTERVAL == 0) and (global_step > 0) and accelerator.is_main_process:

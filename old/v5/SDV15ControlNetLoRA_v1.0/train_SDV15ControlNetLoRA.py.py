"""
# Stable Diffusion v1-5 + ControlNet(Canny) + LoRA(UNet/ControlNet adapter)
# 입력 L(흑백), 출력 ab(컬러/LAB), 저장/평가 RGB 변환.
# MSE(L2) + CLIP + LPIPS + SSIM
- Stable Diffusion v1-5: 텍스트→이미지 생성 기본 프레임워크 (diffusers 라이브러리 사용)
- ControlNet(Canny): 엣지맵(윤곽) 기반으로 이미지 구조 보존, 보조 조건부 제어
- LoRA: UNet/ControlNet에만 어댑터 삽입, lightweight 미세조정
- 프롬프트 강화: PromptEnhancer, DynamicParameterGenerator로 품질/색감/조명/길이/가이드 스케일 등 강화 및 최적화
- 입력: L채널(흑백, LAB), 출력: ab채널(컬러, LAB), 최종 결과는 RGB로 변환 후 저장/평가
-------------------------------------------
[파인튜닝/저장/추론/학습 정책]
1. LoRA 파인튜닝 파라미터
    - UNet, ControlNet 모듈에만 LoRA adapter 적용(파라미터 업데이트)
    - VAE, TextEncoder, CLIP 등 나머지 모듈은 모두 freeze(동결)
    - optimizer/gradient update는 LoRA 파라미터만 (filter(lambda p: p.requires_grad, ...))

2. 저장/불러오기 정책
    - 학습 중 오직 LoRA adapter weights만 별도 저장 (save_pretrained())
    - 최신/베스트/체크포인트 모두 LoRA로만 저장 (full pipeline/model 저장 X, 용량 절약)
    - 재학습/추론 시 base 모델과 LoRA weights만 조합하여 로드

3. Validation/추론 파이프라인
    - 검증/샘플 생성 시 base pipeline에 LoRA 적용 상태로 추론
    - 필요 시 native torch 모델로 unwrap 사용
    - 샘플 저장: [입력 | 결과 | GT] 이미지를 하나로 붙여서 저장

4. 손실 함수
    - 학습(Train) 시: MSE(L2) 또는 L1 Loss만 실제 optimizer/bp에 사용 (가장 robust, 수렴 용이)
    - 검증(Validation) 시: MSE/L1, CLIP, LPIPS, SSIM 등
        · CLIP: 이미지-텍스트 의미적 일치도
        · LPIPS: Perceptual(지각적) 유사도
        · SSIM: 구조적 유사도
    - loss_total = mse_loss (+ optional alpha*clip_loss 등 실험적 가중합)
   
5. 프롬프트/캡션 처리
    - NSFW 등 위험 문구 치환, 반복/중복 프롬프트 방지
    - 동적 파라미터(guidance, step, canny 등)는 PromptEnhancer, DynamicParameterGenerator로 자동화
    - 토크나이저로 token 수 제한(프롬프트 잘림 방지)

6. 학습/재개/조기종료
    - patience 기반 early stopping
    - resume from checkpoint (LoRA만 이어받기)
    - global_step, epoch 등도 재개 시 맞춰서 복구
-----------------------------------------
- Full model 저장/불러오기는 금지! (항상 base + LoRA만 따로 저장/조합)
- 추론 시에도 base + LoRA weights만 조합하기 
- 프롬프트 클린/강화, 다중 손실(특히 CLIP/LPIPS/SSIM), 동적 하이퍼파라미터 적용 유지
- robust(에러 방지, 환경 차이 대응, 경량화) 우선
- 0-1 정규화 맞추기
"""
